...
def f1_score_CLS(prediction, ground_truth):
    ...
    recall = 1.0 * num_same / len(ground_truth_tokens)
    f1 = (2 * precision * recall) / (precision + recall)
    return num_same, precision, recall, f1
...
def evaluate_cls(dataset, predictions, total_eval_loss, total_words):
    ...
    for data, pred in zip(dataset, predictions):
        ...
        if pred == None:
            pred = 'BLANK'
        prediction = pred
        tmp_exact_match, tmp_precision, tmp_recall, tmp_f1 = f1_score_CLS(prediction, ground_truths)
        precision += tmp_precision
        recall += tmp_recall
        exact_match += tmp_exact_match
        f1 += tmp_f1
        total += 1
    exact_match = 100.0 * exact_match / total_words
    precision = 100.0 * precision / total_words
    recall = 100.0 * recall / total_words
    f1 = 100.0 * f1 / total_words
