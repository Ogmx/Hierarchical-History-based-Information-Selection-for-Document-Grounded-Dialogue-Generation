...
class MultiBartQA:
    def __init__(self):
        ...
        self.generator = HHDKS_Enc_np_Dec_p_BartForConditionalGeneration.from_pretrained_multi(self.args, self.args.model_file_path, use_pretrain=True)
    ...
    def encode(self, his_ids, his_mask, doc_ids, doc_mask):
        ...
        return his_reps, doc_reps, his_utt
    ...
    def train(self):
        ...
        cached_features_devfile = os.path.join(
                self.args.data_dir,
                "cached_Bart_{}_DOHA_HHDKS_dataset_{}_task_{}_dev_srcLen{}_tgtLen{}".format(
        ...
        cached_features_trainfile = os.path.join(
                self.args.data_dir,
                "cached_Bart_{}_DOHA_HHDKS_dataset_{}_task_{}_train_srcLen{}_tgtLen{}".format(
                    self.tokenizer.__class__.__name__,
                    self.args.data_set,
                    self.args.experiment_type,
                    str(self.args.source_max_len),
        ...
        if self.args.data_set == "cmu":
            ...
            dev_examples = self.load_examples(self.args.data_dir, 'wow_test_unseen_hislen_3.csv')
            train_examples = self.load_examples(self.args.data_dir, 'wow_train_hislen_3.csv')
        ...
        for epoch in trange(int(self.args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                all_his_ids = torch.cat([his3_ids, his2_ids, his1_ids], dim=-1)
                all_his_mask = torch.cat([his3_mask, his2_mask, his1_mask], dim=-1)
                all_his_reps, doc_reps, his_utt_reps = self.encode(all_his_ids, all_his_mask, doc_ids, doc_mask)
                his_utt_mask = torch.ones((his_utt_reps.shape[0], his_utt_reps.shape[1])).to(self.device).int()
                outputs = self.generator(input_ids=None,
                                         attention_mask=(his_utt_mask, doc_mask),
                                         encoder_outputs=(his_utt_reps, doc_reps),
    ...
    def generate(self):
        ...
        if self.args.experiment_type == 'chat_wizard':
            ...
            cached_features_testfile = os.path.join(
                    self.args.data_dir,
                    "cached_Bart_{}_DOHA_HHDKS_dataset_{}_task_{}_test_srcLen{}_tgtLen{}".format(
                        self.tokenizer.__class__.__name__,
                        self.args.data_set,
                        self.args.experiment_type,
                        str(self.args.source_max_len), 
    ...
    def generate_wizard(self):
        ...
        cached_features_testfile = os.path.join(
                self.args.data_dir,
                "cached_Bart_{}_DOHA_HHDKS_dataset_{}_task_{}_test_seen_srcLen{}_tgtLen{}".format(
                    self.tokenizer.__class__.__name__,
                    self.args.data_set,
                    self.args.experiment_type,
                    str(self.args.source_max_len), 
        ...
        test_examples = self.load_examples(self.args.data_dir, 'wow_test_seen_hislen_3.tsv')
        if os.path.exists(cached_features_testfile):
            test_features = torch.load(cached_features_testfile)
        else:
            test_features = self.convert_examples_to_features(test_examples)
        ...
        cached_features_testfile = os.path.join(
                self.args.data_dir,
                "cached_Bart_{}_DOHA_HHDKS_dataset_{}_task_{}_test_unseen_srcLen{}_tgtLen{}".format(
                    self.tokenizer.__class__.__name__,
                    self.args.data_set,
                    self.args.experiment_type,
                    str(self.args.source_max_len), 
        ...
        test_examples = self.load_examples(self.args.data_dir, 'wow_test_unseen_hislen_3.tsv')
        if os.path.exists(cached_features_testfile):
            test_features = torch.load(cached_features_testfile)
        else:
            test_features = self.convert_examples_to_features(test_examples)
